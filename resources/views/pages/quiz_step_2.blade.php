...
<div class="bg-section">
    <div class="w-full custom-container flex flex-wrap bg-left-bottom bg-contain bg-no-repeat"
        ...
        <div class="w-full md:w-1/2 bg-white shadow-lg rounded-lg p-6 md:p-10">
            ...
            <form action="{{ route('quiz.step3') }}" method="POST" class="space-y-6 quiz-step3">
                @csrf
                <div class="space-y-4">
                ...
                @if (session()->has('customsId'))
                    <input type="hidden" class="customs_id" name="customs_id" value="{{ session('customsId') }}">
                @endif
...
<script>
    $(document).ready(function() {
        $('input[name="level"]').change(function() {
            ...
            let customsId = $(this).closest('form').find('.customs_id').val();
            localStorage.setItem('customsId', JSON.stringify(customsId));
